  ...
  <div>
    <div v-if="$store.state.searchSongs && $store.state.searchSongs.length > 0">
      <div class="edit">
        ...
        <div class="round">
          <el-button type="primary" round icon="el-icon-document-copy" @click="add(1)">添加到播放列表</el-button>
          <el-button type="success" round icon="el-icon-folder-add" @click="add(2)">添加到试听列表</el-button>
        </div>
      </div>
      <el-table
        ref="searchTable"
      ...
      <el-alert type="success" :closable="false" center effect="dark" v-if="isLoading || isEnd">
        <template slot="title" v-if="isLoading"> <i class="el-icon-loading"></i> 正在努力加载... </template>
        <template slot="title" v-else-if="isEnd"> <i class="el-icon-finished"></i> 到底啦... </template>
      </el-alert>
    </div>
    <template v-else>
      <h1>还没有搜索关键词哦</h1>
